...
public interface EmployeeRepository extends JpaRepository<Employee, Long> {
    ...
    @Query(nativeQuery = true, value = "SELECT * FROM employee WHERE " + "LOWER(name) LIKE LOWER(CONCAT('%', :search, '%')) OR " + "LOWER(department) LIKE LOWER(CONCAT('%', :search, '%')) OR " + "CAST(salary AS CHAR) LIKE CONCAT('%', :search, '%') OR " + "CAST(id AS CHAR) LIKE CONCAT('%', :search, '%') ")
