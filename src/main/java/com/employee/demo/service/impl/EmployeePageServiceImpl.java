...
public class EmployeePageServiceImpl implements EmployeePageService{
	@Autowired
	private EmployeePageRepository employeePageReository;
	 ...
	 @Override
	    public EmployeePageResponse getEmployeesWithPaginationAndSorting(int page, int size, String sortBy, String sortDirection, String keyword) {
	        ...
	        if(keyword != null && !keyword.isEmpty()) {
	        		employeePage = employeePageReository.findByNameContainingIgnoreCase(keyword, pageable);            
	        }
	        else {
	         employeePage = employeePageReository.findAll(pageable);
	        }
