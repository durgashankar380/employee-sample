...
//	}
	    ...
	    try {
	        ...
	        for (Employee emp : employees) {
	            if (emp.getDepartment() != null) { 
	                ...
	                departmentSalaryMap.put(departmentName, departmentSalaryMap.getOrDefault(departmentName, 0.0) + salary);
	            }
	        }
	...
	public Set<String> getUniqueDepartments() {
		...
		for (Employee employee : employees) {
			uniqueDepartments.add(employee.getDepartment().getName());
		}
...
//	}
	...
	public Map<String, List<ResponseEmployee>> getTop3EmployeesByDepartment() {
	    try {
	        ...
	        for (Employee emp : employees) {
	            departmentMap.computeIfAbsent(emp.getDepartment().getName(), k -> new ArrayList<>()).add(emp);
	        }
	        ...
	        for (Map.Entry<String, List<Employee>> entry : departmentMap.entrySet()) {
	            ...
	            for (int i = 0; i < Math.min(3, employeeList.size()); i++) {
	                ...
	                top3.add(new ResponseEmployee(emp.getId(), emp.getName(), emp.getSalary(), emp.getStatus(), emp.getDepartment().getName()));
	            }
    ...
    public List<ResponseEmployee> getEmployeesWithSecondHighestSalary() {
        try {
            ...
            double secondHighestSalary = distinctSalaries.get(1);

            
            return employees.stream()
                    .filter(emp -> Double.compare(emp.getSalary(), secondHighestSalary) == 0)
                    .map(ResponseEmployee::new)
                    .collect(Collectors.toList());
    ...
    public String getDepartmentWithHighestTotalSalary() {
        try {
            ...
            for (Employee Employee : Employees) {
                departmentGradeSum.put(Employee.getDepartment().getName(),
    ...
    public List<ResponseEmployee> getEmployeesAboveDepartmentAverage() {
        try {
            ...
            // Step 1: Calculate total salary and count for each department
            for (Employee employee : employees) {
                String departmentName = employee.getDepartment().getName();
                departmentSalarySum.put(departmentName, departmentSalarySum.getOrDefault(departmentName, 0.0) + employee.getSalary());
                departmentCount.put(departmentName, departmentCount.getOrDefault(departmentName, 0) + 1);
            }

            // Step 2: Compute average salary for each department
            Map<String, Double> departmentAverage = new HashMap<>();
            for (String dept : departmentSalarySum.keySet()) {
                departmentAverage.put(dept, departmentSalarySum.get(dept) / departmentCount.get(dept));
            }

            // Step 3: Filter employees whose salary is above department average
            List<ResponseEmployee> result = new ArrayList<>();
            for (Employee employee : employees) {
                String departmentName = employee.getDepartment().getName();
                Double avgSalary = departmentAverage.get(departmentName);

                // Only add employee if salary is strictly greater than department average
                if (avgSalary != null && employee.getSalary() > avgSalary) {
                    result.add(new ResponseEmployee(employee.getId(), 
                            employee.getName(), 
                            employee.getSalary(),
                            employee.getStatus(),
    ...
    public ResponseEmployee getEmployeeById(Long id) {
        Employee employee = employeeRepository.findById(id)
                .orElseThrow(() -> new EmployeeNotFoundException(ApiStatus.EMPLOYEE_NOT_FOUND));
        ...
        return new ResponseEmployee(
                ...
                employee.getSalary(),
                employee.getStatus(),
                employee.getDepartment().getName() // If you need department name
        );
    }
