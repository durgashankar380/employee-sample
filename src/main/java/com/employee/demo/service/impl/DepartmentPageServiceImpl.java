...
public class DepartmentPageServiceImpl implements DepartmentPageService{
    ...
    public ResponseEntity<?> getPage(EmployeePageRequest request) {
        ...
        pageable= PageRequest.of(request.getPageIndex(),
                size,
                Sort.by(sort,sortBy));
    ...
    public ResponseEntity<?> manageStatus(Long id, Integer status) {
        ...
        if(oldDepartment==null){
            ...
            if(status==1)return ResponseEntity.ok(APIStatus.DEPARTMENT_STATUS_ACTIVATED_SUCCESSFULLY);

            if(status==2)return ResponseEntity.ok(APIStatus.DEPARTMENT_STATUS_DEACTIVATED_SUCCESSFULLY);

            return ResponseEntity.ok(APIStatus.DEPARTMENT_STATUS_TEMPORARILY_DELETED_SUCCESSFULLY);

        }
        return  ResponseEntity.badRequest().body(APIStatus.INVALID_STATUS);
